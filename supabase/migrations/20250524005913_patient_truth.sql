/*
  # Fix approval functions and policies

  1. New Functions
    - Improved `approve_letter_with_signature` function with proper error handling
    - Enhanced `get_letter_by_request_id` function with better security
    - Enhanced `get_letter_details_for_approval` function with better security
  
  2. Security
    - Updated policies for approvers to view and update letters
    - Added policies for approval logs
*/

-- حذف الدالة الحالية للموافقة
DROP FUNCTION IF EXISTS approve_letter_with_signature(uuid, uuid, text);

-- حذف الدالة الحالية لجلب الخطاب بواسطة طلب الموافقة
DROP FUNCTION IF EXISTS get_letter_by_request_id(uuid);

-- حذف الدالة الحالية لجلب تفاصيل الخطاب للموافقة
...
-- إنشاء دالة محسنة لجلب الخطاب بواسطة طلب الموافقة
...
-- إنشاء دالة محسنة لجلب تفاصيل الخطاب للموافقة
