/*
  # وظائف تتبع الوقت ومعالجة المهام
  
  1. New Functions
    - `get_task_time_records` - استرجاع سجلات تتبع الوقت للمهمة
    - `update_task_status` - تحديث حالة المهمة مع التحقق من الصلاحيات
    - `add_task_comment` - إضافة تعليق للمهمة
    - `save_task_time_record` - تسجيل وقت للمهمة
...
-- Function to update task status with reason tracking
