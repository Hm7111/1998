/*
  ...
  1. New Functions
    - `get_task_time_records`: Retrieves time records for a task
    - `update_task_status`: Updates task status with appropriate logging
    - `add_task_comment`: Adds a comment to a task
    - `save_task_time_record`: Saves a time record entry for a task
    
  2. Security
    - All functions use SECURITY DEFINER
    - All functions verify user access rights
*/

-- Drop existing functions first to avoid return type conflicts
DROP FUNCTION IF EXISTS get_task_time_records(uuid);
DROP FUNCTION IF EXISTS update_task_status(uuid, task_status, timestamptz);
DROP FUNCTION IF EXISTS add_task_comment(uuid, uuid, text);
DROP FUNCTION IF EXISTS save_task_time_record(uuid, uuid, integer, text);

-- Function to get time records for a task
CREATE OR REPLACE FUNCTION get_task_time_records(p_task_id uuid)
RETURNS TABLE (
  id uuid,
  task_id uuid,
  user_id uuid,
