/*
  # Create missing task management functions

  1. Functions Created
    - `get_task_time_records` - retrieves time tracking records for a specific task
    - `update_task_status` - updates task status and logs the change
    - `add_task_comment` - adds a comment to a task
    - `save_task_time_record` - saves a time tracking record for a task

  2. Purpose
    - Provide RPC functions for task management operations
    - Ensure proper logging of task changes
    - Handle time tracking functionality
    - Support task commenting system

  3. Security
    - Functions use proper row-level security checks
    - Only authorized users can access task data
    - All changes are properly logged
*/

-- Function to get time records for a task
CREATE OR REPLACE FUNCTION get_task_time_records(task_id uuid)
RETURNS TABLE (
  id uuid,
  task_id uuid,
  user_id uuid,
...
BEGIN
  ...
  WHERE tl.task_id = get_task_time_records.task_id
